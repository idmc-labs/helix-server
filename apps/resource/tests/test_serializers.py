...
from utils.factories import CountryFactory, ResourceFactory, ResourceGroupFactory
...
class TestResourceSerializer(HelixTestCase):
    def setUp(self):
        ...
        self.resource = ResourceFactory.create_batch(RESOURCE_NUMBER - 1, created_by=self.user, group=self.group)
    ...
    def test_resource_limit_creation_by_user(self):
        ...
        serializer = ResourceSerializer(data=data1, context={"request": self.request})
        ...
        serializer = ResourceSerializer(data=data2, context={"request": self.request})
