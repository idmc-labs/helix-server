...
class ResourceSerializer(MetaInformationSerializerMixin, serializers.ModelSerializer):
    ...
    def validate(self, attrs) -> dict:
        if (
            ...
            and Resource.objects.filter(created_by=self.context["request"].user).count() >= RESOURCE_NUMBER
        ):
...
class ResourceGroupSerializer(MetaInformationSerializerMixin, serializers.ModelSerializer):
    ...
    def validate(self, attrs) -> dict:
        if (
            self.instance is None
            and ResourceGroup.objects.filter(created_by=self.context["request"].user).count() >= RESOURCEGROUP_NUMBER
        ):
