    ...
    def is_deletable(self) -> bool:
        if res := self.resources.count():
            raise ProtectedError(
                gettext("There are %d resource(s) associated to this group. Please delete them first") % res, []
            )
        return True
...
class Resource(MetaInformationArchiveAbstractModel):
    ...
    countries = models.ManyToManyField("country.Country", verbose_name=_("Countries"), related_name="+", blank=False)
    last_accessed_on = models.DateTimeField(verbose_name=_("Last Accessed On"), blank=True, null=True)
