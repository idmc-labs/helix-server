...
class TestExtractionFilter(HelixTestCase):
    ...
    def setUpTestData(cls) -> None:
        ...
        cls.event1crisis1 = EventFactory.create(crisis=cls.crisis1, event_type=Crisis.CRISIS_TYPE.CONFLICT)
        cls.event1crisis1.countries.set([cls.country2reg2])

        cls.event2crisis1 = EventFactory.create(crisis=cls.crisis1, event_type=Crisis.CRISIS_TYPE.DISASTER)
        cls.event2crisis1.countries.set([cls.country1reg1])

        cls.event3crisis2 = EventFactory.create(crisis=cls.crisis2, event_type=Crisis.CRISIS_TYPE.CONFLICT)
    ...
    def test_filter_by_figure_violence_types(self):
        ...
        # field (a 500 for any client using it). It must filter on Figure.violence.
        ...
        # entry with a conflict figure whose violence matches -> included
        self.assertIn(entry_match.id, ids)
        # entry whose only conflict figure has a different violence -> excluded
    ...
    def test_filter_by_crises(self):
        data = dict(filter_figure_crises=[self.crisis1.id])
        fqs = f(data=data).qs
    ...
    def test_filter_by_publishers(self):
        ...
        data = dict(filter_entry_publishers=[self.org3.id])
        fqs = f(data=data).qs
        self.assertEqual(set(fqs), {self.entry3event2})

        data = dict(filter_entry_publishers=[self.org2.id])
        fqs = f(data=data).qs
    ...
    def test_filter_by_time_frame(self):
        ...
        self.fig1cat1entry1 = FigureFactory.create(
            entry=self.entry1event1,
            category=self.fig_cat1,
            start_date=self.mid_oct,
            end_date=self.end_oct,
        ...
        self.fig2cat2entry1 = FigureFactory.create(
            ...
            start_date=self.end_oct,
            end_date=self.end_nov,
            event=self.random_event,
            figure_cause=Crisis.CRISIS_TYPE.OTHER,
        )
        self.fig3cat2entry2 = FigureFactory.create(
            entry=self.entry2event2,
            category=self.fig_cat2,
            start_date=self.mid_sep,
            end_date=self.end_oct,
            event=self.random_event,
            figure_cause=Crisis.CRISIS_TYPE.OTHER,
        )
        self.fig4cat1entry3 = FigureFactory.create(
            entry=self.entry3event2,
            category=self.fig_cat1,
            start_date=self.mid_nov,
            end_date=None,
            event=self.random_event,
            figure_cause=Crisis.CRISIS_TYPE.OTHER,
        )
    ...
    def test_filter_by_tags(self):
        data = dict(filter_figure_tags=[self.tag1.id])
        fqs = f(data=data).qs
        self.assertEqual(set(fqs), {self.entry1event1})
        data = dict(filter_figure_tags=[self.tag3])
        fqs = f(data=data).qs
        self.assertEqual(set(fqs), {self.entry2event2})

    def test_filter_by_categories(self):
        data = dict(filter_figure_categories=[self.fig_cat1])
        fqs = f(data=data).qs
        self.assertEqual(set(fqs), {self.entry1event1, self.entry3event2})
        data = dict(filter_figure_categories=[self.fig_cat2])
        fqs = f(data=data).qs
        self.assertEqual(set(fqs), {self.entry1event1, self.entry2event2})

    def test_filter_by_category_types(self):
        data = dict(filter_figure_category_types=["FLOW"])
        fqs = f(data=data).qs
        self.assertEqual(set(fqs), {self.entry1event1, self.entry2event2})
        data = dict(filter_figure_category_types=["STOCK"])
        fqs = f(data=data).qs
        self.assertEqual(set(fqs), {self.entry1event1, self.entry3event2})
        ...
        assert (
            all(
                [
                    ...
                    for figure in [
                        self.figure1entry1event1,
                        self.figure2entry2event2,
                        self.fig1cat1entry1,
                        self.fig2cat2entry1,
                    ]
                ]
