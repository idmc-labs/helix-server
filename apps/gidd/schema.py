...
def idps_stock_year(qs, end_year):
    """The single year IDP stock (total_displacement) is read from.

    Stock is a point-in-time (end-of-year) figure, so it is taken from exactly the
    requested ``end_year`` and never summed across years. When ``end_year`` is
    omitted, fall back to the latest year that has stock in ``qs`` (the
    release-capped latest available year) rather than summing every year. Callers
    still sum within that one year across cause / trigger type / country. Returns
    the year, or ``None`` when ``qs`` has no stock at all.
    """
    if end_year:
        return end_year
    return qs.filter(total_displacement__gt=0).aggregate(year=models.Max("year"))["year"]
...
class Query(graphene.ObjectType):
    ...
    def resolve_gidd_public_conflict_statistics(parent, info, **kwargs):
        ...
        conflict_stock_year = idps_stock_year(conflict_qs, end_year)
    ...
    def resolve_gidd_public_disaster_statistics(parent, info, **kwargs):
        ...
        disaster_stock_year = idps_stock_year(disaster_qs, end_year)
    ...
    def resolve_gidd_public_event(parent, info, **kwargs):
        ...
        # Sourced from GiddEventDisplacement across ALL causes (conflict + disaster), not just
        # disasters. One event spans multiple country rows; aggregate over all of them.
        event_qs = GiddEventDisplacementFilter(data=kwargs).qs.filter(event_raw_id=event_id)

        if not event_qs.exists():
            return None

        base = event_qs.values("event_name", "start_date", "end_date").first()
        total_new_displacement = event_qs.aggregate(total=models.Sum("new_displacement"))["total"]

        # event_codes are country-scoped on GiddEventDisplacement; union them to an event-level set.
        event_codes, event_codes_type, seen = [], [], set()
        for codes, code_types in event_qs.values_list("event_codes", "event_codes_type"):
            for code, code_type in zip(codes or [], code_types or []):
                if (code, code_type) not in seen:
                    seen.add((code, code_type))
                    event_codes.append(code)
                    event_codes_type.append(code_type)
    ...
    def resolve_gidd_public_combined_statistics(parent, info, **kwargs):
        ...
        disaster_total_displacement_qs = disaster_base.filter(
            total_displacement__gt=0, year=idps_stock_year(disaster_base, end_year)
        )
        ...
        conflict_total_displacement_qs = conflict_base.filter(
            total_displacement__gt=0, year=idps_stock_year(conflict_base, end_year)
        )
    ...
    def resolve_gidd_public_country_displacements(parent, info, **kwargs):
        ...
        # total_displacement is IDP stock (point-in-time) -> the requested end_year's value
        # (or the latest available year when end_year is omitted), summed across sub types
        # within that one year, never across years.
        end_year = kwargs.get("end_year")
        conflict_stock_year = idps_stock_year(qs.filter(conflict_filter), end_year)
        disaster_stock_year = idps_stock_year(qs.filter(disaster_filter), end_year)
