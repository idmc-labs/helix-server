...
class Report(MetaInformationArchiveAbstractModel, QueryAbstractModel, FigureDisaggregationAbstractModel, models.Model):
    ...
    generated_from = enum.EnumField(REPORT_TYPE, blank=True, null=True, editable=False)
    # TODO: Remove me after next migration run
    generated = models.BooleanField(verbose_name=_("Generated"), default=True, editable=False)
    reports = models.ManyToManyField(
        "self", verbose_name=_("Reports (old groups)"), blank=True, related_name="masterfact_reports", symmetrical=False
    )
    # Do not access me, instead access report_figures property
    figures = models.ManyToManyField("entry.Figure", blank=True)
    ...
    # user entered fields
    analysis = models.TextField(verbose_name=_("Analysis"), blank=True, null=True)
    methodology = models.TextField(verbose_name=_("Methodology"), blank=True, null=True)
    significant_updates = models.TextField(verbose_name=_("Significant Updates"), blank=True, null=True)
    ...
    summary = models.TextField(
        ...
        null=True,
        help_text=_(
            "It will store master fact information:"
            "Comment, Source Excerpt, IDU Excerpt, Breakdown & "
            "Reliability, and Caveats"
        ),
    )
    is_signed_off = models.BooleanField(default=False)
    is_signed_off_by = models.ForeignKey(
        User,
        verbose_name=_("Last signed off by"),
        blank=True,
        null=True,
        related_name="signed_off_reports",
    ...
    gidd_report_year = models.PositiveIntegerField(verbose_name=_("GIDD report year"), null=True, unique=True)
    gidd_published_date = models.DateTimeField(verbose_name=_("Date of data publication into the GIDD"), null=True)
    is_pfa_published_in_gidd = models.BooleanField(verbose_name=_("Is PFA published in GIDD"), default=False)
    ...
    def get_excel_sheets_data(cls, user_id, filters):
        ...
        data = (
            ...
            .qs.annotate(
                ...
                iso3=StringAgg(
                    "filter_figure_countries__iso3", EXTERNAL_ARRAY_SEPARATOR, distinct=True, output_field=models.CharField()
                ),
            )
            .order_by("created_at")
        )
        ...
        def transform_filter_figure_category(figure_categories):
            if figure_categories:
                return get_string_from_list(
                    [category.label if category is not None else "" for category in figure_categories]
                )
            return ""
        ...
        def transform_filter_figure_crisis_types(figure_crisis_types):
            if figure_crisis_types:
                return get_string_from_list(
                    [crisis_type.label if crisis_type is not None else "" for crisis_type in figure_crisis_types]
                )
            return ""
    ...
    def generate_remarks_for_report(self):
        ...
        if self.filter_figure_categories is None or (
            bool(set([item.value for item in self.filter_figure_categories]) & set(figure_categories_to_check))
            and (total_masterfact_figures != total_flow_sum and total_masterfact_figures != total_stock_sum)
        ):
    ...
    def sign_off(self, done_by: "User", include_history: bool = False):
        ...
        transaction.on_commit(lambda: trigger_report_generation.delay(current_gen.pk))
...
class ReportApproval(MetaInformationArchiveAbstractModel, models.Model):
    ...
    created_by = models.ForeignKey(User, verbose_name=_("Approved By"), related_name="approvals", on_delete=models.CASCADE)
...
class ReportGeneration(MetaInformationArchiveAbstractModel, models.Model):
    ...
    report = models.ForeignKey("Report", verbose_name=_("Report"), related_name="generations", on_delete=models.CASCADE)
    is_signed_off = models.BooleanField(default=False)
    is_signed_off_on = models.DateTimeField(verbose_name=_("Is signed off on"), null=True)
    is_signed_off_by = models.ForeignKey(
        User,
        verbose_name=_("Is Signed Off By"),
        blank=True,
        null=True,
        related_name="signed_off_generations",
        on_delete=models.CASCADE,
    )
    ...
    def get_snapshot(self):
        ...
        return dict(
            ...
            entries=Entry.objects.filter(id__in=self.report.report_figures.values("entry"))
            ...
            events=Event.objects.filter(id__in=self.report.report_figures.values("event"))
            .select_related(
                "created_by",
                "last_modified_by",
                "violence",
