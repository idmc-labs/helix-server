...
def check_is_pfa_visible_in_gidd(report):
    ...
    if not (report.filter_figure_start_after and report.filter_figure_end_before):
        ...
        if not (
            ...
            and start_date_month == 1
            and start_date_day == 1
            and end_date_month == 12
            and end_date_day == 31
        ):
    ...
    if not report.filter_figure_crisis_types:
        ...
        len(set(report.filter_figure_crisis_types).intersection({Crisis.CRISIS_TYPE.DISASTER, Crisis.CRISIS_TYPE.CONFLICT}))
        != 1
    ):
        errors.append("Report should have conflict or disaster crisis type.")
    ...
    if not report.filter_figure_categories:
        ...
        len(
            set(report.filter_figure_categories).intersection(
                {
...
class ReportSerializer(MetaInformationSerializerMixin, serializers.ModelSerializer):
    ...
    def validate(self, attrs) -> dict:
        if self.instance is not None and not self.has_permission_for_report(
...
class ReportCommentSerializer(MetaInformationSerializerMixin, serializers.ModelSerializer):
    class Meta:
        model = ReportComment
        fields = "__all__"
...
class ReportSignoffSerializer(serializers.Serializer):
    ...
    def validate_report(self, report):
        if not ReportGeneration.objects.filter(report=report, is_signed_off=False).exists():
    ...
    def save(self):
        ...
        report.sign_off(self.context["request"].user, include_history=self.validated_data.get("include_history", False))
        return report


class ReportGenerationSerializer(MetaInformationSerializerMixin, serializers.ModelSerializer):
    class Meta:
        model = ReportGeneration
        fields = ["report"]

    def validate_report(self, report):
        if ReportGeneration.objects.filter(report=report).count() == settings.GRAPHENE_DJANGO_EXTRAS["MAX_PAGE_SIZE"]:
            raise serializers.ValidationError(
                gettext("Report Generation is limited to %(size)s only.")
                % {"size": settings.GRAPHENE_DJANGO_EXTRAS["MAX_PAGE_SIZE"]}
            )
        if ReportGeneration.objects.filter(report=report, is_signed_off=False).exists():
...
class ReportApproveSerializer(serializers.Serializer):
    ...
    def validate_report(self, report):
        if not ReportGeneration.objects.filter(report_id=report, is_signed_off=False).exists():
        ...
        if (
            ...
            raise serializers.ValidationError(
                gettext("Report approvals is limited to %(size)s only.")
                % {"size": settings.GRAPHENE_DJANGO_EXTRAS["MAX_PAGE_SIZE"]}
            )
        return report
