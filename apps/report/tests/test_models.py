...
class TestReportModel(HelixTestCase):
    ...
    def test_002_appropriate_figures_are_summed_up(self):
        ...
        f2.save()
        r = Report(filter_figure_start_after=f1.start_date, filter_figure_end_before=f1.end_date)
        r.save()
        ...
        data = (
            ...
            .union(
                ...
                .annotate(
                    ...
                    iso3=models.F("country__iso3"),
                    total=models.Sum(
                        "disaggregation_conflict_political", filter=models.Q(disaggregation_conflict_political__gt=0)
                    ),
                ...
                .annotate(
                    ...
                    iso3=models.F("country__iso3"),
                    total=models.Sum(
                        "disaggregation_conflict_criminal", filter=models.Q(disaggregation_conflict_criminal__gt=0)
                    ),
                ...
                .annotate(
                    ...
                    iso3=models.F("country__iso3"),
                    total=models.Sum(
                        "disaggregation_conflict_communal", filter=models.Q(disaggregation_conflict_communal__gt=0)
                    ),
...
class TestReportGenerationApproval(HelixTestCase):
    ...
    def test_report_generation_approval_is_created_only_once(self):
        ...
        ReportApproval.objects.create(generation=gen, created_by=reviewer, is_approved=True)
        assert gen.approvers.count() == 1

        try:
            # user disapproves the generation again
            ReportApproval.objects.create(generation=gen, created_by=reviewer, is_approved=False)
