...
class TestCreateReport(HelixGraphQLTestCase):
    ...
    def test_valid_report_creation(self) -> None:
        ...
        self.assertEqual(
            len(content["data"]["createReport"]["result"]["filterFigureCountries"]), len(self.input["filterFigureCountries"])
        )
        self.assertEqual(content["data"]["createReport"]["result"]["changeInSource"], True)
...
class TestReportSignOff(HelixGraphQLTestCase):
    ...
    def test_valid_report_generation_for_sign_off(self):
        ...
        response = self.query(self.generate_mutation, variables=self.variables)
    ...
    def test_valid_report_sign_off_and_retrieval(self):
        ...
        self.assertIn("report", [i["field"] for i in content["data"]["signOffReport"]["errors"]])
...
class TestReportApprove(HelixGraphQLTestCase):
    def setUp(self) -> None:
        self.admin = create_user_with_role(USER_ROLE.ADMIN.name)
        self.report = ReportFactory.create(created_by=self.admin)
        self.generation = ReportGeneration.objects.create(report=self.report)
        self.mutation = """
...
class TestReportComment(HelixGraphQLTestCase):
    def setUp(self) -> None:
        ...
        self.report = ReportFactory.create()
        self.input = dict(report=str(self.report.id), body="lol")
    ...
    def test_valid_commenting(self):
        ...
        self.assertTrue(content["data"]["createReportComment"]["result"]["createdBy"]["email"], user.email)

    def test_valid_comment_update(self):
        user = create_user_with_role(USER_ROLE.MONITORING_EXPERT.name)
        comment = ReportCommentFactory.create(created_by=user)
        input_data = dict(
            body="blaaa",
            id=str(comment.id),
        )
        ...
        self.assertTrue(content["data"]["updateReportComment"]["result"]["createdBy"]["email"], user.email)
...
class TestReportFilter(HelixGraphQLTestCase):
    ...
    def test_filter_report(self):
        ...
        for filter, exp_result in [
            ...
            response = self.query(self.report_list_query, variables={filter: True})
            content = response.json()
...
class TestPaf(HelixGraphQLTestCase):
    ...
    def test_only_admin_can_change_is_pfa_visible_in_gidd(self):
        ...
        response = self.query(
            self.set_pfa_visible_in_gidd, variables={"reportId": self.report.id, "isPfaVisibleInGidd": True}
        )
        ...
        update_response = self.query(self.update_report, input_data={"id": self.report.id, "isPublic": False})
