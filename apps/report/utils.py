...
def report_global_numbers(report):
    ...
    formatted_data = [
        ...
        dict(one="Total Internal Displacements (Conflict + Disaster)", two=data["flow_total"]),
        dict(
            one="Conflict",
        ...
        dict(one="Number of countries with figures", two=data["countries_count"]),
        dict(
            one="Conflict",
...
def report_stat_flow_country(report):
    ...
    formulae = {}
    data = (
        Country.objects.filter(id__in=report.report_figures.values("country"))
        .annotate(
        ...
        .annotate(total=Coalesce(F(Country.ND_CONFLICT_ANNOTATE), 0) + Coalesce(F(Country.ND_DISASTER_ANNOTATE), 0))
        .order_by("id")
        .values(*list(headers.keys()))
    )
    return {
        "headers": headers,
        "data": data,
...
def report_stat_flow_region(report):
    ...
    formulae = {}
    data = (
        CountryRegion.objects.filter(id__in=report.report_figures.values("country__region"))
        .annotate(
        ...
        .annotate(
            total=Coalesce(F(CountryRegion.ND_CONFLICT_ANNOTATE), 0) + Coalesce(F(CountryRegion.ND_DISASTER_ANNOTATE), 0)
        )
        .values(*list(headers.keys()))
    )
    return {
...
def report_stat_conflict_country(report, include_history):
    ...
    formulae = {
        ...
        "ND percent variation wrt last year": EXCEL_FORMULAE["percent_variation"].format(
            key1=get_key("flow_total"), key2=get_key("flow_total_last_year")
        ),
        "ND percent variation wrt average": EXCEL_FORMULAE["percent_variation"].format(
        ...
        "IDPs percent variation wrt last year": EXCEL_FORMULAE["percent_variation"].format(
            key1=get_key("stock_total"), key2=get_key("stock_total_last_year")
        ),
        "IDPs percent variation wrt average": EXCEL_FORMULAE["percent_variation"].format(
            key1=get_key("stock_total"), key2=get_key("stock_historical_average")
        ),
    }
    global_filter = dict(role=Figure.ROLE.RECOMMENDED, event__event_type=Crisis.CRISIS_TYPE.CONFLICT)

    data = (
        report.report_figures.values("country")
        .order_by()
        .annotate(
            ...
            flow_total=Sum(
                "total_figures", filter=Q(category=Figure.FIGURE_CATEGORY_TYPES.NEW_DISPLACEMENT, **global_filter)
            ),
            stock_total=Sum(
                "total_figures",
                filter=Q(
    ...
    if is_grid_or_myu_report(report.filter_figure_start_after, report.filter_figure_end_before) and include_history:
        data = data.annotate(
            flow_total_last_year=Subquery(
                ...
                .annotate(_total=Sum("total_figures"))
                .values("_total")
                .annotate(total=F("_total"))
                .values("total")
            ),
            flow_historical_average=Subquery(
                ...
                .annotate(_total=Sum("total_figures") / (F("max_year") - F("min_year") + 1))
                .values("_total")
                .annotate(total=F("_total"))
                .values("total")
            ),
            stock_total_last_year=Subquery(
                ...
                .filter(
                    Q(end_date__isnull=False, end_date__gte=report.filter_figure_end_before - timedelta(days=365))
...
def report_stat_conflict_region(report, include_history):
    ...
    data = (
        report.report_figures.annotate(region=F("country__region"))
        .values("region")
        .order_by()
        .annotate(
...
def report_stat_conflict_typology(report):
    ...
    filtered_report_figures = (
        report.report_figures.filter(
            role=Figure.ROLE.RECOMMENDED,
            event__event_type=Crisis.CRISIS_TYPE.CONFLICT,
            category=Figure.FIGURE_CATEGORY_TYPES.NEW_DISPLACEMENT,
        )
    ...
    data = (
        filtered_report_figures.filter(disaggregation_conflict__gt=0)
        .annotate(
            name=F("country__idmc_short_name"),
            iso3=F("country__iso3"),
        ...
        .union(
            filtered_report_figures.filter(disaggregation_conflict_political__gt=0)
            .annotate(
                name=F("country__idmc_short_name"),
                iso3=F("country__iso3"),
                total=Sum("disaggregation_conflict_political", filter=Q(disaggregation_conflict_political__gt=0)),
                typology=models.Value("Violence - Political", output_field=models.CharField()),
            )
            .values("name", "iso3", "total", "typology"),
            filtered_report_figures.filter(disaggregation_conflict_criminal__gt=0)
            .annotate(
                name=F("country__idmc_short_name"),
                iso3=F("country__iso3"),
                total=Sum("disaggregation_conflict_criminal", filter=Q(disaggregation_conflict_criminal__gt=0)),
                typology=models.Value("Violence - Criminal", output_field=models.CharField()),
            )
            .values("name", "iso3", "total", "typology"),
            filtered_report_figures.filter(disaggregation_conflict_communal__gt=0)
            .annotate(
                name=F("country__idmc_short_name"),
                iso3=F("country__iso3"),
                total=Sum("disaggregation_conflict_communal", filter=Q(disaggregation_conflict_communal__gt=0)),
                typology=models.Value("Violence - Communal", output_field=models.CharField()),
            )
            .values("name", "iso3", "total", "typology"),
            filtered_report_figures.filter(disaggregation_conflict_other__gt=0)
            .annotate(
                name=F("country__idmc_short_name"),
                iso3=F("country__iso3"),
                total=Sum("disaggregation_conflict_other", filter=Q(disaggregation_conflict_other__gt=0)),
                typology=models.Value("Other", output_field=models.CharField()),
            )
            .values("name", "iso3", "total", "typology"),
        )
        .values("name", "iso3", "typology", "total")
        .order_by("typology")
    )

    # further aggregation
    aggregation_headers = OrderedDict(
        dict(
            typology="Conflict typology",
            total="Sum of figure",
        )
...
def report_disaster_event(report):
    ...
    # NOTE: {{ }} turns into { } after the first .format
    global_filter = dict(role=Figure.ROLE.RECOMMENDED, event__event_type=Crisis.CRISIS_TYPE.DISASTER)

    data = (
        report.report_figures.filter(**global_filter)
        .values("event")
        .order_by()
        .annotate(
            event_id=F("event_id"),
...
def report_disaster_region(report, include_history):
    ...
    data = (
        report.report_figures.filter(**global_filter)
        .annotate(region=F("country__region"))
