...
class TestLogin(HelixGraphQLTestCase):
    ...
    @override_settings(MAX_LOGIN_ATTEMPTS=1)
    @mock.patch("apps.users.serializers.validate_hcaptcha")
...
class TestChangePassword(HelixGraphQLTestCase):
    ...
    def test_valid_password_change(self):
        ...
        response = self.query(
            self.change_query,
            variables={"oldPassword": self.user.raw_password, "newPassword": newpass},
        )
...
class TestActivate(HelixGraphQLTestCase):
    def setUp(self) -> None:
        ...
        """
        self.input = {"uid": encode_uid(self.user.pk), "token": default_token_generator.make_token(self.user)}
...
class TestUserListSchema(HelixGraphQLTestCase):
    ...
    def test_filter_users(self):
        ...
        self.assertResponseNoErrors(response)
        self.assertEqual(
            sorted([int(each["id"]) for each in content["data"]["users"]["results"]]), sorted([ue.id, ur.id, ua.id])
        )
        ...
        content = response.json()
        self.assertResponseNoErrors(response)
        self.assertEqual(sorted([int(each["id"]) for each in content["data"]["users"]["results"]]), sorted([ur.id, ue.id]))
    ...
    def test_users_fields_access_based_on_authentication(self):
        ...
        self.force_login(guest)
        response = self.query(users_q)
        content = response.json()
        self.assertResponseNoErrors(response)
        self.assertEqual(content["data"]["users"]["totalCount"], total_user_count)
        self.assertEqual(set([item["email"] for item in content["data"]["users"]["results"]]), set([guest.email, None]))
        self.assertEqual(
            {
                item["id"]: (
        ...
        self.assertIn(None, [item["permissions"] for item in content["data"]["users"]["results"]])
        ...
        self.assertResponseNoErrors(response)
        self.assertEqual(content["data"]["users"]["totalCount"], total_user_count)
        self.assertEqual(set([item["email"] for item in content["data"]["users"]["results"]]), set([urc.email, None]))
        self.force_login(ua)
        response = self.query(users_q)
        content = response.json()
        self.assertResponseNoErrors(response)
        ...
        self.assertIn(None, [item["email"] for item in content["data"]["users"]["results"]])
...
class TestAPIMe(HelixAPITestCase):
    def test_me_api(self):
        user = UserFactory.create(email="ram@gmail.com")
...
class TestForgetResetPassword(HelixGraphQLTestCase):
    ...
    def test_user_can_reset_password(self):
        ...
        response = self.query(
            self.reset_password_query,
            variables={"passwordResetToken": token, "newPassword": "12343@#S#", "uid": uid},
        )
        self.assertResponseNoErrors(response)
