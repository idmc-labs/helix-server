...
class TestAdminPortfolioSerializer(HelixTestCase):
    ...
    def test_unique_admin_per_user(self):
        ...
        serializer = AdminPortfolioSerializer(data=data, context=context)
        self.assertTrue(serializer.is_valid(), serializer.errors)
...
class TestRegionalCoordinatorPortfolioSerializer(HelixTestCase):
    ...
    def test_only_admin_is_allowed(self):
        ...
        serializer = RegionalCoordinatorPortfolioSerializer(data=data, context=context)
        self.assertFalse(serializer.is_valid(), serializer.errors)
        self.assertEqual(serializer.errors["non_field_errors"][0].code, "not-allowed", serializer.errors)
...
class TestMonitoringExpertPortfolioSerializer(HelixTestCase):
    ...
    def test_admin_can_create(self):
        ...
        serializer = BulkMonitoringExpertPortfolioSerializer(data=data, context=context)
        self.assertTrue(serializer.is_valid(), serializer.errors)
    ...
    def test_unique_expert_per_country(self):
        ...
        expert = create_user_with_role(USER_ROLE.MONITORING_EXPERT.name, country=country1.id)
        ...
        coordinator = create_user_with_role(
            USER_ROLE.REGIONAL_COORDINATOR.name, monitoring_sub_region=monitoring_sub_region.id
        )
        ...
        self.assertEqual(expert.highest_role, USER_ROLE.GUEST)
        self.assertEqual(guest.highest_role, USER_ROLE.MONITORING_EXPERT)
    ...
    def test_user_roles_are_reset_on_updating_regional_portfolio(self):
        ...
        self.assertEqual(self.coordinator.highest_role, USER_ROLE.GUEST)
        self.assertEqual(guest.highest_role, USER_ROLE.REGIONAL_COORDINATOR)
