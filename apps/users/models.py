        ...
        users = (
            ...
            .annotate(
                portfolio_role=models.Case(
                    ...
                    models.When(
                        portfolio_roles__overlap=[USER_ROLE.MONITORING_EXPERT.value], then=USER_ROLE.MONITORING_EXPERT.value
                    ),
...
class Portfolio(models.Model):
    ...
    def get_coordinator(cls, ms_region: int) -> typing.Optional[Portfolio]:
        ...
        return cls.get_coordinators().filter(monitoring_sub_region=ms_region).first()
    ...
    @property
    def permissions(self) -> list[dict]:
        return [{"action": k, "entities": list(v)} for k, v in PERMISSIONS[USER_ROLE[self.role]].items()]
    ...
    class Meta:
        constraints = [
            UniqueConstraint(
                fields=["role", "monitoring_sub_region"],
                condition=models.Q(role=USER_ROLE.REGIONAL_COORDINATOR),
                name="unique_for_monitoring_sub_region",
            ),
            UniqueConstraint(
                fields=["role", "country"], condition=models.Q(role=USER_ROLE.MONITORING_EXPERT), name="unique_for_country"
            ),
        ]
