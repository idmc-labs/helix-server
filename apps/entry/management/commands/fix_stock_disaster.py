    ...
    def handle(self, *args, **options):
        ...
        with open(csv_file, "r") as csv_file:
            ...
            for figure_id in figures_to_convert_to_recommended_qs.values_list("id", flat=True):
                self.stdout.write(
                    self.style.SUCCESS(f"Updated stock figure {figure_id} with role as recommended and the end date")
                )
            ...
            success = 0
            for figure in Figure.objects.filter(id__in=figures_to_clone_as_stock):
