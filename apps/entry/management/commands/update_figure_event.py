...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--delete-empty-events", action="store_true", help="Delete events that are not associated with any figures"
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        if figure_event_map:
            ...
            self.stdout.write(self.style.ERROR("No figure event to be updated"))
        ...
        if kwargs["delete_empty_events"]:
            ...
            self.stdout.write(self.style.SUCCESS(f"Deleted events: {events_to_be_deleted_stat}"))

            # Events associated with figure will not be deleted
            failed_to_delete_event_stat = event_to_be_deleted_qs.filter(total_figure_count__gt=0).values_list(
                "id", flat=True
            )
            self.stdout.write(self.style.ERROR(f"Failed to delete events: {failed_to_delete_event_stat}"))
