...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        facts = (
            Facts.objects.using("helixmigration")
            .annotate(
                geo_locations=NestableKeyTextTransform("locations", "locations"),
            )
        ...
        for fact in json.loads(serialized_facts):
            ...
            for location_item_to_update in fact["fields"]["locations"]["locations"]:
                if location_item_to_update["moved"]:
                    location_dict["locations"].append(self.get_lat_lon_osm_id_from_locations(location_item_to_update))
