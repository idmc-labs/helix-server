...
class FigureLocation(UUIDAbstractModel, models.Model):
    ...
    country = models.TextField(verbose_name=_("Country"), blank=True, null=True)
    # NOTE: country-code here actually stores iso2
    country_code = models.CharField(verbose_name=_("Country Code"), max_length=8, null=True, blank=False)
    street = models.TextField(verbose_name=_("Street"), blank=True, null=True)
    wiki_data = models.TextField(verbose_name=_("Wiki data"), blank=True, null=True)
    osm_id = models.CharField(verbose_name=_("OSM Id"), max_length=256, blank=True, null=True)
    osm_type = models.CharField(verbose_name=_("OSM Type"), max_length=256, blank=True, null=True)
    house_numbers = models.TextField(verbose_name=_("House numbers"), blank=True, null=True)
    ...
    state = models.TextField(verbose_name=_("State"), blank=True, null=True)
    bounding_box = ArrayField(verbose_name=_("Bounding Box"), base_field=models.FloatField(), blank=True, null=True)
    type = models.TextField(verbose_name=_("Type"), blank=True, null=True)
    importance = models.FloatField(verbose_name=_("Importance"), blank=True, null=True)
    class_name = models.TextField(verbose_name=_("Class"), blank=True, null=True)
    name = models.TextField(verbose_name=_("Name"), blank=True, null=True)
    name_suffix = models.TextField(verbose_name=_("Name Suffix"), blank=True, null=True)
    place_rank = models.IntegerField(verbose_name=_("Place Rank"), blank=True, null=True)
    alternative_names = models.TextField(verbose_name=_("Alternative names"), blank=True, null=True)
    # custom fields
...
class FigureDisaggregationAbstractModel(models.Model):
    ...
    disaggregation_displacement_urban = models.PositiveIntegerField(
        verbose_name=_("Displacement/Urban"), blank=True, null=True
    )
    disaggregation_displacement_rural = models.PositiveIntegerField(
        verbose_name=_("Displacement/Rural"), blank=True, null=True
    )
    disaggregation_location_camp = models.PositiveIntegerField(verbose_name=_("Location/Camp"), blank=True, null=True)
    disaggregation_location_non_camp = models.PositiveIntegerField(
        verbose_name=_("Location/Non-Camp"), blank=True, null=True
    )
    # lesbian, gay, bisexual, transgender, intersex, and questioning (LGBTIQ)
    disaggregation_lgbtiq = models.PositiveIntegerField(verbose_name=_("LGBTIQ+"), blank=True, null=True)
    disaggregation_disability = models.PositiveIntegerField(verbose_name=_("Disability"), blank=True, null=True)
    disaggregation_indigenous_people = models.PositiveIntegerField(
        verbose_name=_("Indigenous people"), blank=True, null=True
    )
    disaggregation_sex_male = models.PositiveIntegerField(verbose_name=_("Sex/Male"), blank=True, null=True)
    disaggregation_sex_female = models.PositiveIntegerField(verbose_name=_("Sex/Female"), blank=True, null=True)
    ...
    disaggregation_strata_json = ArrayField(
        base_field=JSONField(verbose_name=_("Stratum")), verbose_name=_("Strata Disaggregation"), blank=True, null=True
    )
    # conflict based disaggregation
    disaggregation_conflict = models.PositiveIntegerField(verbose_name=_("Conflict/Conflict"), blank=True, null=True)
    disaggregation_conflict_political = models.PositiveIntegerField(
        verbose_name=_("Conflict/Violence-Political"), blank=True, null=True
    )
    disaggregation_conflict_criminal = models.PositiveIntegerField(
        verbose_name=_("Conflict/Violence-Criminal"), blank=True, null=True
    )
    disaggregation_conflict_communal = models.PositiveIntegerField(
        verbose_name=_("Conflict/Violence-Communal"), blank=True, null=True
    )
    disaggregation_conflict_other = models.PositiveIntegerField(verbose_name=_("Other"), blank=True, null=True)
...
class Figure(MetaInformationArchiveAbstractModel, UUIDAbstractModel, FigureDisaggregationAbstractModel, models.Model):
    ...
    event = models.ForeignKey("event.Event", verbose_name=_("Event"), related_name="figures", on_delete=models.CASCADE)
    ...
    sources = models.ManyToManyField(
        "organization.Organization", verbose_name=_("Source"), blank=True, related_name="sourced_figures"
    )
    approved_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="figure_approved_by",
        on_delete=models.SET_NULL,
    )
    approved_on = models.DateTimeField(verbose_name="Assigned at", null=True, blank=True)
    review_status = enum.EnumField(
        enum=FIGURE_REVIEW_STATUS, verbose_name=_("Figure status"), default=FIGURE_REVIEW_STATUS.REVIEW_NOT_STARTED
    )
    ...
    def annotate_stock_and_flow_dates(cls):
        return {
            "flow_start_date": Case(When(category__in=Figure.flow_list(), then=F("start_date"))),
            "flow_end_date": Case(When(category__in=Figure.flow_list(), then=F("end_date"))),
            "flow_start_date_accuracy": Case(When(category__in=Figure.flow_list(), then=F("start_date_accuracy"))),
            "flow_end_date_accuracy": Case(When(category__in=Figure.flow_list(), then=F("end_date_accuracy"))),
            "stock_date": Case(When(category__in=Figure.stock_list(), then=F("start_date"))),
            "stock_reporting_date": Case(When(category__in=Figure.stock_list(), then=F("end_date"))),
            "stock_date_accuracy": Case(When(category__in=Figure.stock_list(), then=F("start_date_accuracy"))),
        }
        ...
        def transformer(datum):
            ...
            return {
                **datum,
                ...
                "quantifier": get_enum_label("quantifier", Figure.QUANTIFIER),
                "unit": get_enum_label("unit", Figure.UNIT),
                "role": get_enum_label("role", Figure.ROLE),
                "term": get_enum_label("term", Figure.FIGURE_TERMS),
                "category": get_enum_label("category", Figure.FIGURE_CATEGORY_TYPES),
                "displacement_occurred": get_enum_label("displacement_occurred", Figure.DISPLACEMENT_OCCURRED),
    ...
    def update_event_status_and_send_notifications(cls, event_id):
        ...
        event_with_stats = Event.objects.filter(id=event_id).annotate(**Event.annotate_review_figures_count()).first()
        ...
        if (
            prev_status != event_with_stats.review_status
            and event_with_stats.review_status == Event.EVENT_REVIEW_STATUS.APPROVED
        ):
...
class EntryReviewer(MetaInformationAbstractModel, models.Model):
    ...
    entry = models.ForeignKey("Entry", verbose_name=_("Entry"), related_name="reviewing", on_delete=models.CASCADE)
    reviewer = models.ForeignKey(
        "users.User", verbose_name=_("Reviewer"), related_name="reviewing", on_delete=models.CASCADE
    )
    status = enum.EnumField(enum=REVIEW_STATUS, verbose_name=_("Review Status"), default=REVIEW_STATUS.TO_BE_REVIEWED)
    ...
    def assign_creator(cls, entry: "Entry", user: "User") -> None:
        cls.objects.filter(entry=entry, created_by__isnull=True).update(created_by=user)

    def update_status(self, status: REVIEW_STATUS) -> None:
        if status == self.REVIEW_STATUS.SIGNED_OFF and not self.reviewer.has_perms(("entry.sign_off_entry",)):
...
class Entry(MetaInformationArchiveAbstractModel, models.Model):
    ...
    preview = models.ForeignKey(
        ...
        null=True,
        help_text=_(
            "After the preview has been generated pass its id"
    ...
    document_url = models.URLField(verbose_name=_("Document URL"), max_length=2000, blank=True, null=True)
    article_title = models.TextField(verbose_name=_("Event Title"))
    publishers = models.ManyToManyField(
        "organization.Organization", verbose_name=_("Publisher"), blank=True, related_name="published_entries"
    )
    publish_date = models.DateField(verbose_name=_("Published Date"))

    idmc_analysis = models.TextField(
        verbose_name=_("Trends and patterns of displacement to be highlighted"), blank=True, null=True
    )
        ...
        def transformer(datum):
            return {
                **datum,
                ...
                "categories": get_string_from_list([category.label if category else "" for category in datum["categories"]]),
                "terms": get_string_from_list([term.label if term else "" for term in datum["terms"]]),
    ...
    def latest_reviews(self):
        return self.reviews.order_by(*Review.UNIQUE_TOGETHER_WITHOUT_ENTRY_FIELDS, "-created_at").distinct(
