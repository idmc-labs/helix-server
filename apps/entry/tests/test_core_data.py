    ...
    return {
        "disaster_idps": get_safe_sum(
            [
                ...
                if (
                    ...
                    and figure.category == Figure.FIGURE_CATEGORY_TYPES.IDPS
                    and figure.role == Figure.ROLE.RECOMMENDED
                )
            ]
        ),
        "disaster_nds": get_safe_sum(
            [
                ...
                if (
                    figure.event.event_type == Crisis.CRISIS_TYPE.DISASTER
                    and figure.category == Figure.FIGURE_CATEGORY_TYPES.NEW_DISPLACEMENT
                    and figure.role == Figure.ROLE.RECOMMENDED
                )
            ]
        ),
...
class TestCoreData(HelixGraphQLTestCase):
    ...
    def init_data(cls):
        ...
        report_data_set: typing.List[typing.Tuple[int, typing.Union[None, Country]]] = [
            (year, country) for year in range(start_year, end_year + 1) for country in [None, *countries]
        ]
        for year, country in report_data_set:
            ...
            report_name = f"GRID {year + 1}" if country is None else f"GRID {year + 1} - {country.iso3}"
        ...
        cls.figures = list(Figure.objects.all().select_related("event", "event__crisis", "country"))
            ...
            for country in self.countries:
                ...
                for figure in self.figures:
                    if (
                        ...
                        and figure.category in Figure.flow_list()
                        and check_date_range_inside_date_range(
                            figure.start_date,
                            figure.end_date,
                            date(year, 1, 1),
                            date(year, 12, 31),
                        ...
                        figure.country == country
                        and figure.category in Figure.stock_list()
                        and check_same_date(figure.end_date, date(year, 12, 31))
                    ):
                        filtered_figures.append(figure)
    ...
    def test_figure_aggregation_in_events(self):
        ...
        for event in self.all_events:
            ...
            filtered_figures = []
            for figure in self.figures:
                if figure.event == event and figure.category in Figure.flow_list():
                    filtered_figures.append(figure)
                elif (
                    max_date
                    and figure.event == event
                    and figure.category in Figure.stock_list()
                    and check_same_date(figure.end_date, max_date)
                ):
                    filtered_figures.append(figure)
    ...
    def test_figure_aggregation_in_crises(self):
        ...
        for crisis in self.all_crises:
            ...
            filtered_figures = []
            for figure in self.figures:
                if figure.event.crisis == crisis and figure.category in Figure.flow_list():
                    filtered_figures.append(figure)
                elif (
                    max_date
                    and figure.event.crisis == crisis
                    and figure.category in Figure.stock_list()
                    and check_same_date(figure.end_date, max_date)
                ):
                    filtered_figures.append(figure)
        ...
        for report in self.reports:
            ...
            for figure in self.figures:
                if (
                    ...
                    and (not country or figure.country == country)
                ):
                    filtered_figures.append(figure)
                elif (
                    figure.category in Figure.stock_list()
                    and check_same_date(
                        figure.end_date,
                        report.filter_figure_end_before,
                    )
            ...
            report_name = (
                f"GRID {report.filter_figure_start_after.year + 1}"
                if country is None
                else f"GRID {report.filter_figure_start_after.year + 1} - {country.iso3}"
            )
    ...
    def test_crisis_inclusion_in_country(self):
        for country in self.countries:
            ...
            for crisis in self.all_crises:
                ...
                filtered_figures = []
                for figure in self.figures:
                    if figure.country == country and figure.event.crisis == crisis and figure.category in Figure.flow_list():
                        filtered_figures.append(figure)
                    elif (
                        figure.country == country
                        and max_date
    ...
    def test_event_inclusion_in_country(self):
        for country in self.countries:
            ...
            for event in self.all_events:
                ...
                filtered_figures = []
                for figure in self.figures:
                    if figure.country == country and figure.event == event and figure.category in Figure.flow_list():
                        filtered_figures.append(figure)
                    elif (
                        figure.country == country
                        and max_date
    ...
    def test_event_inclusion_in_crisis(self):
        for crisis in self.all_crises:
            ...
            for event in self.all_events:
                ...
                filtered_figures = []
                for figure in self.figures:
                    if figure.event.crisis == crisis and figure.event == event and figure.category in Figure.flow_list():
                        filtered_figures.append(figure)
                    elif (
                        figure.event.crisis == crisis
                        and max_date
        ...
        for report in self.reports:
            ...
            for figure in self.figures:
                if (
                    ...
                    figure.category in Figure.stock_list()
                    and check_date_inside_date_range(
                        figure.end_date,
                        report.filter_figure_start_after,
                        report.filter_figure_end_before,
                    )
            ...
            for country in self.countries:
                ...
                for figure in self.figures:
                    if (
                        ...
                        and (not report_country or figure.country == report_country)
                    ):
                        filtered_figures.append(figure)
                    elif (
                        figure.country == country
    ...
    def test_gidd_displacement_data(self):
        ...
        assert combined_stats_r_data["internalDisplacements"] == get_safe_sum(
            [x for x in [overall_disaster_nds, overall_conflict_nds] if x is not None]
        )
    ...
    def test_gidd_disaster_data(self):
        disaster_data = []
        disaster_events = [event for event in self.events if event.event_type == Crisis.CRISIS_TYPE.DISASTER]
        for year in range(self.start_year, self.end_year + 1):
            for country in self.countries:
    ...
    def test_chart_aggregations(self):
        figures = [
            {
                "date": (figure.start_date if figure.start_date.year == figure.end_date.year else figure.end_date)
                if figure.category == Figure.FIGURE_CATEGORY_TYPES.NEW_DISPLACEMENT
                else figure.end_date,
            ...
            if (
                ...
                and figure.category in [Figure.FIGURE_CATEGORY_TYPES.NEW_DISPLACEMENT, Figure.FIGURE_CATEGORY_TYPES.IDPS]
            )
        ]
        ...
        idps_disaster_figures = aggregate_by_date(
            [
                f
                for f in figures
                if f["cause"] == Crisis.CRISIS_TYPE.DISASTER and f["category"] == Figure.FIGURE_CATEGORY_TYPES.IDPS
            ]
        )
        idps_conflict_figures = aggregate_by_date(
            [
                f
                for f in figures
                if f["cause"] == Crisis.CRISIS_TYPE.CONFLICT and f["category"] == Figure.FIGURE_CATEGORY_TYPES.IDPS
            ]
        )
        nds_disaster_figures = aggregate_by_date(
            [
                f
                for f in figures
                if f["cause"] == Crisis.CRISIS_TYPE.DISASTER
                and f["category"] == Figure.FIGURE_CATEGORY_TYPES.NEW_DISPLACEMENT
            ]
        )
        nds_conflict_figures = aggregate_by_date(
            [
                f
                for f in figures
                if f["cause"] == Crisis.CRISIS_TYPE.CONFLICT
                and f["category"] == Figure.FIGURE_CATEGORY_TYPES.NEW_DISPLACEMENT
            ]
        )
