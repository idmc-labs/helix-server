...
class TestRelationLoaderEngine(HelixGraphQLTestCase):
    ...
    def test_all_autowired_fields_route_through_loaders(self) -> None:
        ...
        # 125, not develop's 126: GiddReleaseMetadataType pins Meta.fields, which drops its
        # modified_by resolver -- that FK is how an unauthenticated caller reached UserType.
        ...
        # 83, not develop's 87: the four `disasters` reverses go with the Disaster model. The
        # four `legacy_disasters` stay, because DisasterLegacy is retained, and CountryType
        # excludes the two new gidd_* reverses rather than exposing them unpaginated.
