...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        update = Event.objects.filter(Q(disaster_sub_type__isnull=False)).update(
        ...
        update = Figure.objects.filter(Q(disaster_sub_type__isnull=False)).update(
        ...
        update = Event.objects.filter(Q(disaster_type__isnull=False)).update(
        ...
        update = Figure.objects.filter(Q(disaster_type__isnull=False)).update(
            disaster_sub_category=Subquery(
        ...
        update = Event.objects.filter(Q(disaster_sub_category__isnull=False)).update(
        ...
        update = Figure.objects.filter(Q(disaster_sub_category__isnull=False)).update(
            disaster_category=Subquery(
        ...
        update = Figure.objects.filter(violence_sub_type__isnull=False).update(
        ...
        update = Event.objects.filter(violence_sub_type__isnull=False).update(
