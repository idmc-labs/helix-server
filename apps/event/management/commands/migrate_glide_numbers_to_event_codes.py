...
def migrate_glide_numbers_to_event_codes():
    ...
    event_qs = Event.objects.exclude(
        models.Q(glide_numbers__isnull=True) | models.Q(glide_numbers=[]) | models.Q(event_code__isnull=False)
    )
    ...
    for event in event_qs.values("id", "name", "glide_numbers"):
        ...
        for glide_no in glide_numbers:
            ...
            if not is_code_in_format:
                invalid_glide_numbers.append({**base_error, "reason": f"Invalid GLIDE Number: {glide_no}"})
                continue

            iso3 = glide_no.split("-")[-1]
            country = country_iso3_map.get(str(iso3), None)
            if not country:
                invalid_glide_numbers.append({**base_error, "reason": f"Invalid ISO3: {iso3}"})
                continue

            event_countris_iso3_list = list(
                Event.countries.through.objects.filter(event=event["id"]).values_list("country__iso3", flat=True)
            )

            if iso3 not in event_countris_iso3_list:
                invalid_glide_numbers.append({**base_error, "reason": f"ISO3 not in event: {iso3}"})
                continue

            EventCode.objects.get_or_create(country=country, event_id=event["id"], event_code=glide_no, event_code_type=1)
