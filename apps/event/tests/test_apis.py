...
class TestDataMigrationTestCase(HelixGraphQLTestCase):
    ...
    def test_event_merge(self):
        ...
        self.assertEqual(
            set(Figure.objects.exclude(event_id=self.event3.id).values_list("event_id", flat=True)), {self.event1.id}
        )

        # check for failed figure update
        self.assertNotEqual(BulkApiOperation.objects.first().failure_list, [])
        ...
        for report in Report.objects.all():
            report_filter_figure_events_ids.extend(list(report.filter_figure_events.all().values_list("id", flat=True)))
        self.assertEqual(set(report_filter_figure_events_ids), {self.event1.id})
        self.assertEqual(Report.objects.filter(filter_figure_events=self.event2).count(), 0)

        # test event unified review comment.
        self.assertEqual(set(UnifiedReviewComment.objects.values_list("event_id", flat=True)), {self.event1.id})
...
class TestCreateEventHelixGraphQLTestCase(HelixGraphQLTestCase):
    ...
    def test_valid_event_creation_with_other_sub_type(self) -> None:
        ...
        self.assertIsNone(content["data"]["createEvent"]["errors"], content)
        self.assertEqual(content["data"]["createEvent"]["result"]["name"], self.input["name"])
        self.assertEqual(content["data"]["createEvent"]["result"]["otherSubType"], None)
        ...
        self.assertIsNone(content["data"]["createEvent"]["errors"], content)
        self.assertEqual(content["data"]["createEvent"]["result"]["name"], self.input["name"])
        self.assertNotEqual(content["data"]["createEvent"]["result"]["otherSubType"], None)
...
class TestUpdateEvent(HelixGraphQLTestCase):
    def setUp(self) -> None:
        ...
        self.event_code1 = EventCodeFactory.create(
            event=self.event, country=country1, event_code_type=EventCode.EVENT_CODE_TYPE.GLIDE_NUMBER, event_code="Code-1"
        )
    ...
    def test_valid_event_update(self) -> None:
        ...
        self.assertEqual(content["data"]["updateEvent"]["result"]["name"], self.input["name"])
...
class TestDeleteEvent(HelixGraphQLTestCase):
    ...
    def test_valid_event_delete(self) -> None:
        ...
        self.assertEqual(content["data"]["deleteEvent"]["result"]["name"], self.event.name)
        self.assertEqual(int(content["data"]["deleteEvent"]["result"]["id"]), self.event.id)
...
class TestEventListQuery(HelixGraphQLTestCase):
    ...
    def test_event_list_filter(self):
        ...
        variables = {"crisisByIds": [str(event1.crisis.id)]}
    ...
    def test_event_has_mutiple_recommended_figures(self):
        ...
        geo_location = FigureLocationFactory.create(name="random location")
        for i in range(3):
            entry1 = EntryFactory.create()
        ...
        variables = {"qaRule": QA_RULE_TYPE.HAS_NO_RECOMMENDED_FIGURES.name}
        response = self.query(self.q, variables=variables)
        content = response.json()
        self.assertEqual(content["data"]["eventList"]["totalCount"], 0)

        # Test event with mutiple figures
        variables = {"qaRule": QA_RULE_TYPE.HAS_MULTIPLE_RECOMMENDED_FIGURES.name}
        response = self.query(self.q, variables=variables)
        content = response.json()
    ...
    def test_should_ignore_events_form_qa_if_ignore_qs_flag_is_true(self):
        ...
        geo_location = FigureLocationFactory.create(name="random location2")
        for i in range(3):
            entry1 = EntryFactory.create()
            FigureFactory.create(
                entry=entry1,
