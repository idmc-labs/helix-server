...
class EventTotalFigureDisaggregationLoader(DataLoader):
    """Both current-year figure-disaggregation totals in ONE batched, set-based CTE
    aggregation keyed by event id. Replaces the two per-field loaders that each
    recomputed both totals via the correlated subquery (2x redundant work + 2 round-trips)."""

    def batch_load_fn(self, keys):
        rows = Event.annotate_total_figure_disaggregation_via_cte(Event.objects.filter(id__in=keys), keys=keys).values(
            "id",
...
class MaxStockIDPFigureEndDateByEventLoader(DataLoader):
    def batch_load_fn(self, keys):
        qs = Event.objects.filter(id__in=keys).annotate(**Event._total_figure_disaggregation_subquery())
        batch_load = {
...
class EventEntryCountLoader(DataLoader):
    def batch_load_fn(self, keys):
        qs = Event.objects.filter(id__in=keys).annotate(
        ...
        batch_load = {item["id"]: item["entry_count"] for item in qs.values("id", "entry_count")}
        return Promise.resolve([batch_load.get(key) for key in keys])
...
class EventTypologyLoader(DataLoader):
    def batch_load_fn(self, keys: list):
        qs = (
            Event.objects.filter(id__in=keys)
            .annotate(
                event_typology=Case(
        ...
        batch_load = {item["id"]: item["event_typology"] for item in qs}
        return Promise.resolve([batch_load.get(key) for key in keys])
...
class EventFigureTypologyLoader(DataLoader):
    def batch_load_fn(self, keys: list):
        qs = (
            Figure.objects.filter(event_id__in=keys)
            .values("event_id")
            .annotate(
                event_typology=ArrayAgg(
                    Case(
                        When(other_sub_type__isnull=False, then=F("other_sub_type__name")),
                        When(violence_sub_type__isnull=False, then=F("violence_sub_type__name")),
                        When(disaster_sub_type__isnull=False, then=F("disaster_sub_type__name")),
        ...
        batch_load = {item["event_id"]: item["event_typology"] for item in qs}
        return Promise.resolve([batch_load.get(key) for key in keys])
...
class EventReviewCountLoader(DataLoader):
    def batch_load_fn(self, keys: list):
        qs = (
            Event.objects.filter(id__in=keys)
            .annotate(**Event.annotate_review_figures_count())
            .values(
                "id",
