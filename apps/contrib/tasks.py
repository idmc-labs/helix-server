...
def generate_external_endpoint_dump_file(
    ...
    try:
        data = get_data(filters={**(base_filters or {}), "include_sources": include_sources})
...
def _generate_idus_dump_file(api_type):
    ...
    if api_type == ExternalApiDump.ExternalApiType.IDUS_ALL:
        ...
        # generate geojson dump file with out source
        generate_external_endpoint_dump_file(
            ExternalApiDump.ExternalApiType.IDUS_ALL,
            FigureReadOnlySerializer,
        ...
        # generate geojson dump file with source
        generate_external_endpoint_dump_file(
            ExternalApiDump.ExternalApiType.IDUS_ALL,
            FigureReadOnlySerializer,
            get_idu_data_geojson,
...
@celery_app.task
def generate_idus_references_dump_file():
    ...
    try:
        # snake_case throughout, matching the rest of the IDU family: FigureReadOnlySerializer,
        # which every /idus/ dump is built from, publishes snake_case and no camelCase at all.
        ...
        data = {
            ...
            "disaster_sub_types": disaster_sub_types,
            "violence_types": violence_types,
            "violence_sub_types": violence_sub_types,
            "geographical_groups": regions,
            "countries": countries,
        }
