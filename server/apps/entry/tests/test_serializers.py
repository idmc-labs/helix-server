...
class TestEntrySerializer(HelixTestCase):
    def setUp(self) -> None:
        ...
        self.request.user = self.user = create_user_with_role(ROLE.MONITORING_EXPERT_EDITOR.name)
    ...
    def test_entry_creation_create_entry_reviewers(self):
        reviewer1 = create_user_with_role(ROLE.MONITORING_EXPERT_EDITOR.name)
        reviewer2 = create_user_with_role(ROLE.MONITORING_EXPERT_EDITOR.name)
    ...
    def test_entry_update_entry_reviewers(self):
        x = create_user_with_role(ROLE.MONITORING_EXPERT_EDITOR.name)
        y = create_user_with_role(ROLE.MONITORING_EXPERT_EDITOR.name)
        z = create_user_with_role(ROLE.MONITORING_EXPERT_EDITOR.name)
        entry = EntryFactory.create()
        entry.reviewers.set([x, y, z])

        reviewer1 = create_user_with_role(ROLE.MONITORING_EXPERT_EDITOR.name)
        reviewer2 = create_user_with_role(ROLE.MONITORING_EXPERT_EDITOR.name)
        reviewer3 = create_user_with_role(ROLE.MONITORING_EXPERT_EDITOR.name)
        entry = EntryFactory.create()
