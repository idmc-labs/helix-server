...
def serializer_error_to_error_types(errors: dict, initial_data: dict = None) -> List:
    ...
    for field, value in errors.items():
        if isinstance(value, dict):
            error_types.append(
                _CustomErrorType(field=_camelize_django_str(field), object_errors=serializer_error_to_error_types(value))
            )
            ...
            if isinstance(value[0], str):
                if isinstance(initial_data.get(field), list):
                        ...
                        _CustomErrorType(
                            ...
                            array_errors=[
                                ArrayNestedErrorType(
                                    key=ARRAY_NON_MEMBER_ERRORS, messages="".join(str(msg) for msg in value)
                                )
                            ],
                        )
                    )
                    ...
                    error_types.append(
                        _CustomErrorType(field=_camelize_django_str(field), messages="".join(str(msg) for msg in value))
                    )
                ...
                for pos, array_item in enumerate(value):
                    ...
                    array_errors.append(
                        ArrayNestedErrorType(
                            key=key, object_errors=serializer_error_to_error_types(array_item, initial_data[field][pos])
                        )
                    )
                error_types.append(_CustomErrorType(field=_camelize_django_str(field), array_errors=array_errors))
        else:
            # fallback
            error_types.append(
                _CustomErrorType(field=_camelize_django_str(field), messages=" ".join(str(msg) for msg in value))
